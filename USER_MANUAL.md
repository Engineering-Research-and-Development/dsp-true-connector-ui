...
- **Statistics Dashboard**: Landing page overview of negotiation, transfer, event, and runtime statistics
...
The Statistics Dashboard is the landing page shown after login. It provides a single-page overview of connector activity: KPI cards (total negotiations, total transfers, events processed in the selected window, process CPU usage), negotiation and transfer counts-by-state charts with role/state breakdown tables, a transfer format breakdown and download-flag status cards, a historical events timeline, and a runtime panel (CPU, heap/non-heap memory, live thread count, and uptime).
